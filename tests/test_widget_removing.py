...
async def test_many_remove_all_widgets():
    ...
    async with App().run_test() as pilot:
        await pilot.app.mount(*[Static() for _ in range(300)])
        assert len(pilot.app.screen.children) == 300
...
async def test_many_remove_some_widgets():
    ...
    async with App().run_test() as pilot:
        await pilot.app.mount(*[Static(classes=f"is-{n%2}") for n in range(300)])
        assert len(pilot.app.screen.children) == 300
        await pilot.app.query(".is-0").remove()
        assert len(pilot.app.screen.children) == 150
