...
async def test_single_keypress_printable_character():
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.press("x")
        assert text_editor.text == "x" + TEXT
...
async def test_tab_with_spaces_goes_to_tab_stop(
    ...
    async with app.run_test() as pilot:
        ...
        text_editor.cursor_location = (0, cursor_column)

        await pilot.press("tab")

        assert text_editor.cursor_location[1] == cursor_destination
...
async def test_delete_left_end():
    ...
    async with app.run_test() as pilot:
        text_editor = app.query_one(TextEditor)
        text_editor.load_text("Hello, world!")
        text_editor.move_cursor((0, 13))
        await pilot.press("backspace")
        assert text_editor.text == "Hello, world"
        assert text_editor.selection == Selection.cursor((0, 12))
...
async def test_delete_right():
    ...
    async with app.run_test() as pilot:
        ...
        assert text_editor.text == "Hello, world!"
        assert text_editor.selection == Selection.cursor((0, 13))
...
async def test_delete_right_end_of_line():
    ...
    async with app.run_test() as pilot:
        text_editor = app.query_one(TextEditor)
        text_editor.load_text("hello\nworld!")
        end_of_line = text_editor.get_cursor_line_end_location()
        text_editor.move_cursor(end_of_line)
        await pilot.press("delete")
        assert text_editor.selection == Selection.cursor((0, 5))
        assert text_editor.text == "helloworld!"
...
async def test_delete_line(selection, expected_result):
    ...
    async with app.run_test() as pilot:
        ...
        assert text_editor.selection == Selection.cursor((0, 0))
        assert text_editor.text == expected_result
...
async def test_delete_line_multiline_document(selection, expected_result):
    ...
    async with app.run_test() as pilot:
        text_editor = app.query_one(TextEditor)
        text_editor.load_text("012\n345\n678\n9\n")
        text_editor.selection = selection

        await pilot.press("ctrl+x")

        cursor_row, _ = text_editor.cursor_location
        assert text_editor.selection == Selection.cursor((cursor_row, 0))
        assert text_editor.text == expected_result
...
async def test_delete_to_end_of_line(selection, expected_result):
    ...
    async with app.run_test() as pilot:
        ...
        assert text_editor.selection == Selection.cursor(selection.end)
        assert text_editor.text == expected_result
...
async def test_delete_word_left(selection, expected_result, final_selection):
    ...
    async with app.run_test() as pilot:
        text_editor = app.query_one(TextEditor)
        text_editor.load_text("  012 345 6789")
        text_editor.selection = selection

        await pilot.press("ctrl+w")

        assert text_editor.text == expected_result
        assert text_editor.selection == final_selection
...
async def test_delete_word_left_to_start_of_line():
    ...
    async with app.run_test() as pilot:
        text_editor = app.query_one(TextEditor)
        text_editor.load_text("0123\n   456789")
        text_editor.selection = Selection.cursor((1, 3))

        await pilot.press("ctrl+w")

        assert text_editor.text == "0123\n456789"
...
async def test_delete_word_left_at_line_start():
    ...
    async with app.run_test() as pilot:
        text_editor = app.query_one(TextEditor)
        text_editor.load_text("0123\n   456789")
        text_editor.selection = Selection.cursor((1, 0))

        await pilot.press("ctrl+w")

        assert text_editor.text == "0123   456789"
...
async def test_delete_word_right_delete_to_end_of_line():
    ...
    async with app.run_test() as pilot:
        text_editor = app.query_one(TextEditor)
        text_editor.load_text("01234\n56789")
        text_editor.selection = Selection.cursor((0, 3))

        await pilot.press("ctrl+f")

        assert text_editor.text == "012\n56789"
...
async def test_delete_word_right_at_end_of_line():
    ...
    async with app.run_test() as pilot:
        text_editor = app.query_one(TextEditor)
        text_editor.load_text("01234\n56789")
        text_editor.selection = Selection.cursor((0, 5))
