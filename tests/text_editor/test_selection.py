...
async def test_cursor_location_get():
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        text_editor.selection = Selection((1, 1), (2, 2))
        assert text_editor.cursor_location == (2, 2)
...
async def test_cursor_location_set():
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        target = (1, 2)
        text_editor.cursor_location = target
        assert text_editor.selection == Selection.cursor(target)
...
async def test_cursor_location_set_while_selecting():
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        text_editor.selection = Selection((0, 0), (0, 2))
        target = (1, 2)
        text_editor.cursor_location = target
        assert text_editor.selection == Selection((0, 0), target)
...
async def test_move_cursor_select():
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        text_editor.selection = Selection((1, 1), (2, 2))
        text_editor.move_cursor((2, 3), select=True)
        assert text_editor.selection == Selection((1, 1), (2, 3))
...
async def test_move_cursor_relative():
    ...
    async with app.run_test():
        ...
        text_editor.move_cursor_relative(rows=1, columns=2)
        assert text_editor.selection == Selection.cursor((1, 2))

        text_editor.move_cursor_relative(rows=-1, columns=-2)
        assert text_editor.selection == Selection.cursor((0, 0))

        text_editor.move_cursor_relative(rows=1000, columns=1000)
...
async def test_selected_text_forward():
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        text_editor.selection = Selection((0, 0), (2, 0))
        assert (
...
async def test_selected_text_backward():
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        text_editor.selection = Selection((2, 0), (0, 0))
        assert (
            text_editor.selected_text
            == """\
I must not fear.
...
async def test_selected_text_multibyte():
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        text_editor.load_text("こんにちは")
        text_editor.selection = Selection((0, 1), (0, 3))
        assert text_editor.selected_text == "んに"
...
async def test_selection_clamp():
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        text_editor.selection = Selection((99, 99), (100, 100))
        assert text_editor.selection == Selection(start=(4, 0), end=(4, 0))
...
async def test_get_cursor_left_location(start, end):
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        text_editor.move_cursor(start)
        assert text_editor.get_cursor_left_location() == end
...
async def test_get_cursor_right_location(start, end):
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        text_editor.move_cursor(start)
        assert text_editor.get_cursor_right_location() == end
...
async def test_get_cursor_up_location(start, end):
    ...
    async with app.run_test():
        ...
        text_editor.record_cursor_width()
        assert text_editor.get_cursor_up_location() == end
...
async def test_get_cursor_down_location(start, end):
    ...
    async with app.run_test():
        ...
        # last location navigated to (0, 0)
        text_editor.record_cursor_width()
        assert text_editor.get_cursor_down_location() == end
...
async def test_cursor_word_left_location(start, end):
    ...
    async with app.run_test():
        text_editor = app.query_one(TextEditor)
        text_editor.load_text("AB CD  EFG\n    HI\tJK  LM ")
        text_editor.move_cursor(start)
        assert text_editor.get_cursor_word_left_location() == end
...
async def test_cursor_word_right_location(start, end):
    ...
    async with app.run_test():
        ...
        text_editor.load_text("AB CD  EFG\n    HI\tJK  LM ")
        text_editor.move_cursor(start)
        assert text_editor.get_cursor_word_right_location() == end
...
async def test_select_all(content, expected_selection):
    ...
    async with app.run_test():
        ...
        assert text_editor.selection == expected_selection
...
async def test_select_line(index, content, expected_selection):
    ...
    async with app.run_test():
        ...
        text_editor.select_line(index)

        assert text_editor.selection == expected_selection


async def test_cursor_screen_offset_and_terminal_cursor_position_update():
    class TextEditorCursorScreenOffset(App):
        def compose(self) -> ComposeResult:
            yield TextEditor("abc\ndef")

    app = TextEditorCursorScreenOffset()
    async with app.run_test():
        text_editor = app.query_one(TextEditor)

        assert app.cursor_position == (3, 0)

        text_editor.cursor_location = (1, 1)

        assert text_editor.cursor_screen_offset == (4, 1)
...
async def test_cursor_screen_offset_and_terminal_cursor_position_scrolling():
    ...
    async with app.run_test(size=(80, 2)) as pilot:
        ...
        text_editor.cursor_location = (5, 0)
        await pilot.pause()

        assert text_editor.cursor_screen_offset == (3, 1)
