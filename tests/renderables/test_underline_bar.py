...
from textual.renderables.bar import RenderableBar
...
def test_highlight_half_start():
    bar = RenderableBar(highlight_range=(2.5, 4), width=6)
    assert render(bar) == (
...
def test_highlight_half_end():
    bar = RenderableBar(highlight_range=(2, 4.5), width=6)
    assert render(bar) == (
        f"{GREY}━{STOP}"
        f"{GREY}╸{STOP}"
        f"{MAGENTA}━━{STOP}"
...
def test_highlight_half_start_and_half_end():
    bar = RenderableBar(highlight_range=(2.5, 4.5), width=6)
    assert render(bar) == (
...
def test_highlight_to_near_end():
    bar = RenderableBar(highlight_range=(3, 5.5), width=6)
    assert render(bar) == (
...
def test_highlight_to_end():
    bar = RenderableBar(highlight_range=(3, 6), width=6)
    assert render(bar) == (f"{GREY}━━{STOP}{GREY}╸{STOP}{MAGENTA}━━━{STOP}")


def test_highlight_out_of_bounds_start():
    bar = RenderableBar(highlight_range=(-2, 3), width=6)
    assert render(bar) == (f"{MAGENTA}━━━{STOP}{GREY}╺{STOP}{GREY}━━{STOP}")


def test_highlight_out_of_bounds_end():
    bar = RenderableBar(highlight_range=(3, 9), width=6)
    assert render(bar) == (f"{GREY}━━{STOP}{GREY}╸{STOP}{MAGENTA}━━━{STOP}")


def test_highlight_full_range_out_of_bounds_end():
    bar = RenderableBar(highlight_range=(9, 10), width=6)
    assert render(bar) == f"{GREY}━━━━━━{STOP}"


def test_highlight_full_range_out_of_bounds_start():
    bar = RenderableBar(highlight_range=(-5, -2), width=6)
    assert render(bar) == f"{GREY}━━━━━━{STOP}"
