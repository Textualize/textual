...
class TextArea(ScrollView, can_focus=True):
    ...
    It is completely independent of wrapping.
    ...
    _wrapped_selection: Reactive[Selection] = reactive(
        Selection(), always_update=True, init=False
    )
    """The selection start and end locations in *wrapped* document space.
    
    If wrapping is disabled, then the wrapped selection will be equal to the (document) selection.
    
    When you move the cursor, you'll see it travel through "wrapped" selection space on screen.
    """
    ...
    """Enable or disable soft wrapping. This will attempt to wrap text on spaces.
    If no spaces are available, the text will hard wrap."""

    indent_width: Reactive[int] = reactive(4)
    """The display width of tab characters."""
    ...
    def scroll_cursor_visible(
        ...
        """Scroll the `TextEditor` such that the cursor is visible on screen.
    ...
    def cursor_location(self) -> Location:
        ...
        This is a utility for accessing the `end` of `TextEditor.selection`.
        """
    ...
    def select_all(self) -> None:
        """Select all the text in the `TextEditor`."""
...
class Edit:
    ...
    def do(self, text_editor: TextArea) -> EditResult:
        """Perform the edit operation.

        Args:
            text_editor: The `TextEditor` to perform the edit on.
        ...
        # This code is mostly handling how we adjust TextEditor.selection
        ...
        selection_start, selection_end = text_editor.selection
        selection_start_row, selection_start_column = selection_start
        selection_end_row, selection_end_column = selection_end

        replace_result = text_editor.document.replace_range(edit_from, edit_to, text)
    ...
    def undo(self, text_editor: TextArea) -> EditResult:
        """Undo the edit operation.

        Args:
            text_editor: The `TextEditor` to undo the insert operation on.
    ...
    def after(self, text_editor: TextArea) -> None:
        """Possibly update the cursor location after the widget has been refreshed.

        Args:
            text_editor: The `TextEditor` this operation was performed on.
        """
        if self._updated_selection is not None:
            text_editor.selection = self._updated_selection
