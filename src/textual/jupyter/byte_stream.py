...
if __name__ == "__main__":
    ...
    class TestParser(ByteStreamParser[tuple[bytes, bytes]]):
        def parse(
            self, on_token: ByteStreamTokenCallback
        ) -> Generator[Awaitable, bytes, None]:
            while True:
                header = yield self.read1()
                payload = yield self.read(3)
                print("header", header)
                print("payload", payload)
                on_token((header, payload))
