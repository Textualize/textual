...
class TextEditorTheme:
    """A theme for the `TextEditor` widget.
    ...
    such in the `TextEditor`, assuming a parser which returns a `heading_content`
    node is used (as will be the case when language="markdown").

    ```
    TextEditorTheme('my_theme', syntax_styles={'heading': Style(color='cyan', bold=True)})
    ```

    We can register this theme with our `TextEditor` using the  [`TextEditor.register_theme`][textual.widgets._text_editor.TextEditor.register_theme] method,
    ...
    @classmethod
    def get_builtin_theme(cls, theme_name: str) -> "TextEditorTheme" | None:
        """Get a `TextEditorTheme` by name.

        Given a `theme_name`, return the corresponding `TextEditorTheme` object.

        Args:
            theme_name: The name of the theme.

        Returns:
            The `TextEditorTheme` corresponding to the name or `None` if the theme isn't
            found.
        """
    ...
    @classmethod
    def builtin_themes(cls) -> list[TextEditorTheme]:
        """Get a list of all builtin TextEditorThemes.

        Returns:
            A list of all builtin TextEditorThemes.
        """
        return list(_BUILTIN_THEMES.values())

    @classmethod
    def default(cls) -> TextEditorTheme:
        """Get the default syntax theme.

        Returns:
            The default TextEditorTheme (probably "monokai").
        """
        return _MONOKAI


_MONOKAI = TextEditorTheme(
    name="monokai",
...
DEFAULT_THEME = TextEditorTheme.get_builtin_theme("monokai")
"""The default TextEditorTheme used by Textual."""
